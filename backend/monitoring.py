...
def register_default_health_checks():
    ...
    def check_email_config():
        ...
        try:
            cfg = get_config()
            email_cfg = cfg.email
            if email_cfg and email_cfg.smtp_username and email_cfg.smtp_password:
                return {"status": "ok", "message": "Email configured", "provider": email_cfg.smtp_server}
            return {"status": "warning", "message": "Email not configured"}
        except Exception as e:
            return {"status": "warning", "message": f"Email check skipped: {e}"}
    ...
    def check_groq_api():
        ...
        if groq_key and not groq_key.startswith("your-"):
            return {"status": "ok", "message": "Groq API configured"}
        return {"status": "warning", "message": "Groq API not configured"}
    ...
    def check_rate_limiter():
        ...
        try:
            ...
            return {"status": "ok", "message": "Rate limiter active", "details": status}
        except Exception as e:
            return {"status": "warning", "message": f"Rate limiter check failed: {e}"}
    
    health_checker.register_check("email", check_email_config)
    health_checker.register_check("groq_api", check_groq_api)
