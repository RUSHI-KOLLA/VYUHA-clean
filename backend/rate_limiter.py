...
login_rate_limiter = RateLimiter(requests_per_minute=10, burst_limit=15)
...
class RateLimitMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next):
        ...
        # Use stricter rate limiting for auth/login endpoints
        if path in ["/auth/login", "/auth/register", "/auth/forgot-password"]:
            allowed, headers = login_rate_limiter.check_rate_limit(request)
        else:
