...
async def execute_query_timetable(college_id: str, day: Optional[str] = None, time_slot: Optional[str] = None, faculty_name: Optional[str] = None, room_name: Optional[str] = None, subject_name: Optional[str] = None, empty_only: bool = False):
    ...
    try:
        ...
        # Step 1.5: Special handling for empty room lookup
        if empty_only or (not fac_id and not room_id and not sub_id and day and formatted_time):
            ...
            # Fetch all rooms in the college
            ...
            # Find ALL slots for today to calculate "until when" it is free
            ...
            # Parse times to find who is occupied right now, and when the next classes are
            ...
            for slot in today_slots:
                ...
                if not s_time_str or not e_time_str: continue
            ...
            for room_name, until_str in empty_rooms[:15]: # Show up to 15 empty rooms
...
def _classify_chat_intent(
    ...
    # 1. Explicit Substitution Keywords (Highest Priority)
    is_explicit_sub = _is_substitution_query_text(message)
    if is_explicit_sub:
        return "substitution"
        
    # 2. Explicit Schedule/Room Keywords
    is_explicit_schedule = any(re.search(pattern, lowered, re.IGNORECASE) for pattern in SCHEDULE_PATTERNS)
    
    # If it's explicitly about rooms, schedules, or time, it's a schedule query.
    # We ONLY treat it as substitution if it's a pronoun follow-up.
    if is_explicit_schedule:
        # Pronoun follow-up check: "can you find a replacement for him?"
        if _message_contains_pronoun_reference(message) and _has_recent_substitution_intent(history, context):
            return "substitution"
        return "schedule"
        
    # 3. Pronoun follow-ups without explicit schedule keywords
    if _message_contains_pronoun_reference(message) and _has_recent_substitution_intent(history, context):
        return "substitution"
...
def _has_substitution_context(message: str, history: Optional[List[dict]], context: Optional[dict], college_id: str = None) -> bool:
    """
    Determines if the current message should be treated as a substitution query 
    based on context and history, avoiding overriding explicit schedule queries.
    """
    lowered = message.lower()
    
    # If the message contains a clear schedule/room/availability keyword,
    # we generally should NOT force it into substitution mode unless a faculty is mentioned.
    is_explicit_schedule = any(re.search(pattern, lowered, re.IGNORECASE) for pattern in SCHEDULE_PATTERNS)
    
    entities = _extract_query_entities(message, college_id)
    
    # If it's an explicit schedule query (e.g. "rooms free today") AND no faculty is mentioned,
    # it is definitely NOT a substitution query, regardless of history.
    if is_explicit_schedule and not entities.get("faculty_name"):
        return False

    # If the user is explicitly naming a faculty and it's NOT a schedule query, 
    # we assume they might be looking for a substitute.
    if not is_explicit_schedule and entities.get("faculty_name"):
        return True

    # Check if we are currently in a substitution "session" (context provided by frontend or memory)
    if context and isinstance(context, dict):
        if str(context.get("substitution_faculty") or "").strip():
            return True
        targets = context.get("substitution_targets") or []
    ...
    # Check if the immediate previous turns were about substitutions
    recent_targets = _extract_recent_substitution_targets(history)
    if recent_targets:
        return True

    if bool(_extract_recent_absent_faculty(history)):
        return True
...
async def _execute_confirmed_action(action_type: str, params: Dict[str, Any], college_id: str) -> str:
    ...
    if action_type == "query_timetable":
        return await execute_query_timetable(
            ...
            empty_only=params.get("empty_only", False)
        )
        ...
        "tools": [
            ...
            },
            {
                ...
                "function": {
                    ...
                    "description": "Answers questions about the timetable/schedule by searching for specific rooms, teachers, days, or times. Can also find empty/available rooms if 'empty_only' is set to true along with a day and time_slot.",
                    "parameters": {
                        "type": "object",
                        "properties": {
                            ...
                            "time_slot": {"type": "string", "description": "The starting time of the class (e.g. 08:00, 09:00). If looking for empty rooms right now, provide the current time slot based on the current time context."},
                            "faculty_name": {"type": "string"},
                            "room_name": {"type": "string", "description": "The room name (e.g. Lab B204, Hall A101)."},
                            "subject_name": {"type": "string"},
                            "empty_only": {"type": "boolean", "description": "Set to true if the user is asking for currently empty, free, or available rooms. Requires 'day' and 'time_slot' to be provided."}
                        }
                    }
                }
            }
...
async def chat_interaction(
    ...
    try:
        ...
        if intent == "schedule":
            # Schedule and room lookups are deterministic. Letting the model choose
            # between timetable and substitution tools caused room/schedule questions
            # to be misrouted when the prior conversation was about replacements.
